...
export function saveMobileFeatured() {
	...
	localStorage.setItem('ms_mobile_featured', JSON.stringify(val));
}

export function saveMobileNav() {
	let val;
	mobileNavConfig.subscribe((v) => (val = v))();
	localStorage.setItem('ms_mobile_nav', JSON.stringify(val));
}

export function resetMobileConfigs() {
	mobileFeaturedConfig.set([...DEFAULT_FEATURED]);
	mobileNavConfig.set([...DEFAULT_NAV]);
	localStorage.setItem('ms_mobile_featured', JSON.stringify(DEFAULT_FEATURED));
	localStorage.setItem('ms_mobile_nav', JSON.stringify(DEFAULT_NAV));
}
